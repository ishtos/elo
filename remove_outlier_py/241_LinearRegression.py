...
def coef_and_intercept(card_id):
    res = {'card_id': card_id}
    x = list(new_merchant_transactions.loc[new_merchant_transactions.card_id == card_id].index)
    for c in columns: 
        y = list(new_merchant_transactions.loc[new_merchant_transactions.card_id == card_id][c]) 
