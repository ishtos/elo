...
union = union.query('0 <= installments and installments <= 12')
...
def aggregate(args):
    ...
    agg_Y.columns = ['union_'+'_'.join(col).strip() for col in agg_Y.columns.values]
    agg_Y = agg_Y.reset_index()
    agg_Y = agg_Y.rename(columns={'union_card_id': 'card_id'})
    ...
    agg_N.columns = ['union_'+'_'.join(col).strip() for col in agg_N.columns.values]
    agg_N = agg_N.reset_index()
    agg_N = agg_N.rename(columns={'union_card_id': 'card_id'})
